...
const navItems: NavItem[] = [
  ...
  { to: '/approvals', label: 'Aprobación', icon: ClipboardCheck },
...
export function Sidebar() {
  ...
  const pendingApprovalsCount = approvalQueue?.length ?? 0;
      ...
      <nav className="flex-1 px-2 py-2">
        <ul className="flex flex-col gap-1">
          {navItems.map(({ to, label, icon: Icon }) => {
            const badge = to === '/approvals' && pendingApprovalsCount > 0 ? pendingApprovalsCount : undefined;
